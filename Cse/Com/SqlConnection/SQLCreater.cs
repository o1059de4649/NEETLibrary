    ...
    public class SQLCreater
    {
        ...
        public static string CreateWhereSQLByDictionary(Dictionary<string, object> dic, string databaseName, string tableName)
        {
            ...
            foreach (var item in dic)
            {
                ...
                {
                    values += $@" {CseCommonMethod.CamelToSnake(item.Key)} = '{item.Value}' ";
                }
                else
                {
                    values += $@" AND {CseCommonMethod.CamelToSnake(item.Key)} = '{item.Value}'";
                }
            }
            var result = values;
            return result;
        ...
        public static string CreateJoinSQLByDictionary(Dictionary<string, object> baseDic, string databaseName, string baseTableName,string targetTableName,bool isLeft = false)
        {
            ...
            foreach (var item in baseDic)
            {
                ...
                {
                    values += $@" {databaseName}.{baseTableName}.{CseCommonMethod.CamelToSnake(item.Key)} = {databaseName}.{targetTableName}.{CseCommonMethod.CamelToSnake(item.Key)}";
                }
                else
                {
                    values += $@" AND {databaseName}.{baseTableName}.{CseCommonMethod.CamelToSnake(item.Key)} = {databaseName}.{targetTableName}.{CseCommonMethod.CamelToSnake(item.Key)} ";
                }
            }
            values += ";";
            var result = values;
        ...
        /// ページャーからSQL生成
        /// </summary>
        /// <param name="pageFilter">ページャー</param>
        /// <returns>SQL</returns>
        public static string CreatePaginationSQLByTSQL(PaginationFilterDto pageFilter) {
            var startIndex = (pageFilter.page - 1) * pageFilter.rowsPerPage;
            var res = $@"OFFSET {startIndex} ROWS FETCH NEXT {pageFilter.rowsPerPage} ROWS ONLY";
            return res;
        }
    }
}
